...
const IMAGE_FIELDS = [
  "image_urls",
  "input_urls",
  "image_input", // Nano Banana 2 и Pro — без этого имени они выглядят как text-to-image
  "input_image",
  "image_url",
  "input_url",
  "imageUrls",
...
export function deriveModelMeta(fields) {
  ...
  const imageField = IMAGE_FIELDS.find((name) => byName.has(name)) || guessImageField(fields, promptField);
...
/**
 * Запасной поиск поля для входной картинки, когда имя не из известного списка.
 *
 * Вендоры называют одно и то же по-своему, и список известных имён всегда отстаёт:
 * из-за отсутствия `image_input` модели Nano Banana 2 и Pro выглядели как
 * text-to-image и молча выпадали из пачки. Поэтому если явного совпадения нет —
 * ищем любое поле, похожее на картинку, отдавая предпочтение массиву (референсов
 * обычно можно передать несколько).
 */
function guessImageField(fields, promptField) {
  const candidates = fields.filter((f) => f.name !== promptField
    && /(image|img|photo|picture|frame)/i.test(f.name)
    && !/(output|result|mask|format|size|resolution|quality|count|num)/i.test(f.name));
  if (candidates.length === 0) return null;
  const array = candidates.find((f) => f.type === "array");
  return (array || candidates[0]).name;
}
