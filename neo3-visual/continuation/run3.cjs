#!/usr/bin/env node
// Переделка «Случайной встречи» v2: страницы 2–6 на GPT Image 2, каждая на своей
// сетке с рамками у всех кадров. Готовая страница выравнивается по полям и сразу
// уходит владельцу в телеграм. Живёт сам по себе (nohup), лог — run3.log.
...
function genPage(n) {
  const prompt = fs.readFileSync(path.join(__dirname, `p${n}b.txt`), 'utf8').trim();
...
(async () => {
  ...
  for (const n of [2, 3, 4, 5, 6]) {
    ...
    if (file) {
      try {
        execFileSync('node', [TG, 'send', file, `Новая вёрстка: страница ${n} из 6 — GPT Image 2, все кадры в рамках`],
  ...
  const msg = failed.length === 0
    ? 'Готово: все 5 страниц в новой вёрстке у тебя. Каждая страница на своей сетке, рамки у всех кадров.'
    : `Новая вёрстка: готовы страницы ${done.join(', ')}. Не вышли: ${failed.join(', ')} — Клод разберётся.`;
